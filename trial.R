...
formatting <- function(x){
   ...
   raw <- as.data.frame(lapply(x, function(x) gsub("|", "/", x, fixed = TRUE)))
   return(raw)
} else {
   Ind <- x[,1]
   raw_data <- x[,3:ncol(x)]
   ...
   raw <- data.frame(Ind, Pop, raw_data)
   
   raw_nopop <- data.frame(Ind, raw_data)
   
   raw_long <- raw_nopop %>% tidyr::pivot_longer(
      !Ind,
      names_to = "rsID",
      values_to = "Count"
   )
   
   raw_long$Allele2 <- raw_long$Count
   raw_long$Count <- stringr::str_sub(raw_long$Count, end = -2) 
   raw_long$Allele2  <- stringr::str_sub(raw_long$Allele2 , 2)
   raw_long$GT <- as.character(interaction(raw_long$Count, raw_long$Allele2, sep = "/"))
   raw_final <- data.frame(raw_long$Sample, raw_long$rsID, raw_long$GT)
   raw_final <- raw_final %>% dplyr::rename(
      Ind = 1,
      ID = 2,
      GT = 3
   ) 
   raw_final <- raw_final %>% tidyr::pivot_wider(Ind, names_from = "ID", values_from = "GT")
   
   raw_reference <- raw[,1:2]
   raw <- raw_final %>% left_join(raw_reference, by = "Ind") %>%
      dplyr::rename(Ind = 1, Pop = 2)
   return(raw)
}
}

raw <- raw %>% mutate(across(everything(), formatting))
...
out_files <- lapply(1:nrow(rep.df), function(i) {
   #k <- rep.df[i, "k"]
   #####################
  ...
  # for(i in 1:nrow(rep.df)){
   ...
   if (!file.exists(final_out)) {
      warning("Missing output file for run: ", i)
      return(NULL)
      return(final_out)
   }
   
   result <- tryCatch(structureRead(final_out), error = function(e) {
      warning("Failed to parse STRUCTURE output: ", i)
      return(NULL)
   })
   result$label <- i
   result
...
final_out <- as.list(list.files(path = "./structure_files", pattern = "\\_f$", full.names = TRUE))

#for (file in final_out){

file = "structure_files/structure_input.k1.r1_f"
